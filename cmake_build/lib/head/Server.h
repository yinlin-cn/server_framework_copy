...
private:
    ...
    // db_credit_gate_ 先声明：析构时最后销毁，业务池线程（可能仍持有 DB_credit_token）先 join。
    std::unique_ptr<DB_credit_gate> db_credit_gate_;            // DB 准入额度
    ...
    std::shared_ptr<DB_waiting_queue> db_waiting_queue_;   // DB 等待队列
